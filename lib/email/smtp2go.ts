...
export async function sendShopNotification(input: ShopNotification) {
  ...
  const logoUrl = `${siteUrl}/brand-email-logo.svg`;
  ...
  return sendTransactionalEmail({
    ...
    html: `<!doctype html><html><body style="margin:0;background:#f3eee3;color:#23261f;font-family:Arial,sans-serif"><div style="max-width:680px;margin:auto;padding:34px 18px"><div style="background:#fffdf8;border:1px solid #d9ceb8;border-radius:14px;padding:42px"><img src="${escapeHtml(logoUrl)}" alt="Art by Elyzaveta" width="224" style="display:block;width:224px;max-width:72%;height:auto;margin:0 auto 30px"><p style="letter-spacing:.2em;font-size:11px;color:#5f6548;text-align:center;margin:0">ART BY ELYZAVETA · ENQUIRY</p><h1 style="font-family:Georgia,serif;font-size:42px;line-height:1.05;font-weight:400;text-align:center;margin:24px 0 30px">${escapeHtml(input.heading)}</h1><div style="height:1px;background:#b69a64;margin:0 18% 28px"></div><table style="border-collapse:collapse;width:100%;border:1px solid #ded8cb">${detailHtml}</table><h2 style="font-family:Georgia,serif;font-size:28px;font-weight:400;text-align:center;margin:32px 0 16px">Message</h2><p style="white-space:pre-wrap;line-height:1.7;background:#fbf8f1;border:1px solid #e5dccb;border-radius:8px;padding:20px">${escapeHtml(input.message)}</p>${linkHtml ? `<div style="margin-top:22px"><strong>Private attachment links · available for 7 days</strong>${linkHtml}</div>` : ""}<div style="height:1px;background:#d9ceb8;margin:30px 0 20px"></div><p style="font-family:Georgia,serif;font-style:italic;text-align:center;color:#6d6b61">Reply directly to this email to contact the customer.</p></div></div></body></html>`,
