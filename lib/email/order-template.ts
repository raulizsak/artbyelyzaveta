...
const formatDate = (input: unknown) => {
  ...
  const normalized = /^\d{4}-\d{2}-\d{2}$/.test(String(input))
    ? `${input}T00:00:00`
    : String(input);
  return formatMelbourneDate(normalized, "long");
};
